...
function removeFirstOccurrences(str, value) {
  ...
  const lead = str.slice(0, index);
  const trail = str.slice(index + value.length);
  if (index < 0) {
    return str;
  }
...
function isPalindrome(str) {
  const rts = str
    ...
    .toLowerCase()
    .replace(/[\s.,!?]/g, '');
  if (str.toLowerCase().replace(/[\s.,!?]/g, '') === rts) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  let result = arraySentence[0].length;
  let word = arraySentence[0];
  for (let i = 1; i < arraySentence.length; i += 1) {
    if (result < arraySentence[i].length) {
      result = arraySentence[i].length;
...
function invertCase(str) {
  ...
  for (let i = 0; i < rts.length; i += 1) {
    if (rts[i] === rts[i].toUpperCase() && rts[i] !== rts[i].toLowerCase()) {
      rts[i] = rts[i].toLowerCase();
    } else if (
      rts[i] === rts[i].toLowerCase() &&
      rts[i] !== rts[i].toUpperCase()
    ) {
      rts[i] = rts[i].toUpperCase();
    }
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < rts.length; i += 1) {
    if (
      (rts[i].charCodeAt() >= 110 && rts[i].charCodeAt() >= 97) ||
      (rts[i].charCodeAt() >= 78 && rts[i].charCodeAt() <= 90)
    ) {
      index = rts[i].charCodeAt() - 13;
    } else if (
      rts[i].charCodeAt() === 33 ||
      rts[i].charCodeAt() === 63 ||
      rts[i].charCodeAt() === 32
    ) {
      index = rts[i].charCodeAt();
    } else {
      index = rts[i].charCodeAt() + 13;
    }
...
function getCardId(value) {
  ...
  let i = -1;
  let count = -1;
  for (let k = 0; k < suits.length; k += 1) {
    for (let j = 0; j < rangs.length; j += 1) {
      const card = rangs[j] + suits[k];
      count += 1;
      if (card === value) {
        i = count;
      }
